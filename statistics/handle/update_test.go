...
func TestStatsLockForDelta(t *testing.T) {
	...
	origStat := h.GetTableStats(tableInfo1)
	...
	require.NoError(t, h.Update(is))
	stats1 := h.GetTableStats(tableInfo1)
	require.Equal(t, origStat, stats1)
	...
	/* no entry in stats_meta */
	rows := testKit.MustQuery(fmt.Sprintf("select count(*) from mysql.stats_meta where table_id = %d", tableInfo1.ID)).Rows()[0][0]
...
func TestStatsLockUnLockForColumnStatsUsage(t *testing.T) {
	store, dom := testkit.CreateMockStoreAndDomain(t)
	tk := testkit.NewTestKit(t, store)
