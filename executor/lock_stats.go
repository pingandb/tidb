...
func (e *LockStatsExec) Next(ctx context.Context, req *chunk.Chunk) error {
	...
	if tableNum > 1 {
		...
		return h.AddLockedTables(tids)
	}

	tbl, err := is.TableByName(e.Tables[0].Schema, e.Tables[0].Name)
	if err != nil {
		return err
	}
	return h.AddLockedTable(tbl.Meta().ID)
...
func (e *UnlockStatsExec) Next(ctx context.Context, req *chunk.Chunk) error {
	...
	if tableNum > 1 {
		...
		return h.RemoveLockedTables(tids)
	}
	...
	return h.RemoveLockedTable(tbl.Meta().ID)
}
